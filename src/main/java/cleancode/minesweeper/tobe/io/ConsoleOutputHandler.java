...
public class ConsoleOutputHandler implements OutputHandler{
    ...
    private String decideCellSignFrom(CellSnapshot snapshot) {
        CellSnapshotStatus cellSnapshotStatus = snapshot.getStatus();
        if (cellSnapshotStatus == CellSnapshotStatus.EMPTY) {
            return EMPTY_SIGN;
        }
        if (cellSnapshotStatus == CellSnapshotStatus.FLAG) {
            return FLAG_SIGN;
        }
        if (cellSnapshotStatus == CellSnapshotStatus.LAND_MINE) {
            return LAND_MINE_SIGN;
        }
        if (cellSnapshotStatus == CellSnapshotStatus.NUMBER) {
            return String.valueOf(snapshot.getNearbyLandMineCount());
        }
        if (cellSnapshotStatus == CellSnapshotStatus.UNCHECKED) {
